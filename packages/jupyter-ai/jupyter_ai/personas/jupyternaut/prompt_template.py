...
- For charts, default to clean templates (e.g., Seaborn or Plotly Express), include descriptive titles/axis labels/legends, and explicitly define any palettes/`color_discrete_map` so legend labels always match the same series colors (use contrasting hues to highlight key series while keeping text readable); when a label names a color (e.g., “Black”, “Pink”), set the series color to that literal value; surface the polished visual in the notebook output. Example prompt fragment: “Plot with Plotly Express using `color_discrete_map={'Series A': 'black', 'Series B': 'pink'}` and set the legend titles to match the series names.”
</instructions>
